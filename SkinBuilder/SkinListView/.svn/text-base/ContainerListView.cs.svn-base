...
namespace ZLIS.SkinBuilder
{
	...
	public class ContainerListView: Control
	{
        ...
        protected ImageObject headerImageObject = new ImageObject();
        protected Color headerSplitColor = Color.FromArgb(0, 0, 240);

        protected ImageObject rowImageObject = new ImageObject();
        ...
        [
        Category("Appearance"),
        Description("Set or Get the header height"),
        DefaultValue(20)
        ]
        ...
        [
        Category("Appearance"),
        Description("Get or Set Background Image"),
        DefaultValue(false)
        ]
        public new Image BackgroundImage
        {
            ...
            set 
            {
                this.backgroundImageObject.NormalBitmap = (Bitmap)value;
                this.Invalidate();
            }
        }
        ...
        [
        Category("Appearance"),
        Description("Get or Set Background Image's split margin"),
        DefaultValue(false)
        ]
        public Padding BackgroundSplitMargin
        {
        ...
        [
        Category("Appearance"),
        Description("Get or Set Header Background Image"),
        DefaultValue(false)
        ]
        public Bitmap HeaderBackgroundImage 
        {
            get { return this.headerImageObject.NormalBitmap; }
            set 
            {
                this.headerImageObject.NormalBitmap = value;
                this.Invalidate();
            }
        }
        ...
        [
        Category("Appearance"),
        Description("Get or Set Header Background highlight image "),
        DefaultValue(false)
        ]
        public Bitmap HeaderBackgroundImageHighlight
        {
            get { return this.headerImageObject.HighlightBitmap; }
            set
            {
                this.headerImageObject.HighlightBitmap = value;
                this.Invalidate();
            }
        }
        ...
        [
        Category("Appearance"),
        Description("Get or Set Header Background press image "),
        DefaultValue(false)
        ]
        public Bitmap HeaderBackgroundImagePress
        {
            get { return this.headerImageObject.DownBitmap; }
            set
            {
                this.headerImageObject.DownBitmap = value;
                this.Invalidate();
            }
        }
        ...
        public Color HeaderSplitLineColor
        {
            get { return this.headerSplitColor; }
            set
            {
                this.headerSplitColor = value;
                this.Invalidate();
            }
        }
        ...
        [
        Category("Appearance"),
        Description("Get or Set Row Background Images "),
        DefaultValue(false)
        ]
        public ImageObject RowImageObject 
        {
            get { return this.rowImageObject; }
            set
            {
                this.rowImageObject = value;
                this.Invalidate();
            }
        }
        ...
        private ContainerListViewItem hoverItem = null;
			...
			if (headerStyle != ColumnHeaderStyle.None)
			{
				if (MouseInRect(e, headerRect) && e.Button == MouseButtons.Left)
				{
					for (int i=0; i<columns.Count; i++)
					{
                        ...
                        else if (MouseInRect(e, columnSizeRects[i]))
                        {
                            colScaleMode = true;
                            colScaleWid = columnRects[i].Width;
                            scaledCol = i;
        ...
        protected override void OnMouseLeave(EventArgs e)
        {
            ...
            foreach (ToggleColumnHeader header in this.columns)
            {
		...
		protected void GenerateColumnRects()
		{
			...
			for (int i=0; i<columns.Count; i++)
			{
                ...
                columnRects[i] = new Rectangle(lwidth, 1, colwid, headerBuffer);
				columnSizeRects[i] = new Rectangle(lwidth+colwid-4, ClientRectangle.Top+1, 4, headerBuffer);
				lwidth += colwid+1;
				allColsWidth += colwid;
			}
		}
		...
		{
			headerRect = new Rectangle(this.ClientRectangle.Left+1 - hscrollBar.Value, this.ClientRectangle.Top+1, this.ClientRectangle.Width-3, headerBuffer);
		}

		protected void GenerateRowsRect()
		{
			rowsRect = new Rectangle(this.ClientRectangle.Left+1-hscrollBar.Value, this.ClientRectangle.Top + (headerStyle == ColumnHeaderStyle.None ? 1 : headerBuffer + 1), this.ClientRectangle.Width-3, this.ClientRectangle.Height-(headerStyle == ColumnHeaderStyle.None ? 1 : headerBuffer + 1));
		}
		...
		protected virtual void DrawBackground(Graphics g, Rectangle r)
		{
            ...
            {
                DrawEngine.DrawRect2(g, this.backgroundImageObject, r, ControlState.Normal);
            }
            else
            {
		...
		protected virtual void DrawHeaders(Graphics g, Rectangle r)
		{
			...
			if (headerStyle != ColumnHeaderStyle.None)
			{
             ...
             //   imgAttr.SetWrapMode(WrapMode.Tile);
                ...
                g.FillRectangle(new SolidBrush(SystemColors.Control), r.Left + 1, r.Top + 1, r.Width - 1, headerBuffer);
				...
				g.Clip = new Region(new Rectangle(r.Left+1, r.Top+1, r.Width-4, r.Top+headerBuffer));
				for (i=0; i<columns.Count; i++)
				{
					if ((lp_scr+last+columns[i].Width > r.Left+2)
						&& (lp_scr+last < r.Left+r.Width-2))
					{
                        Rectangle buttonRect = new Rectangle(lp_scr + last, r.Top + 1, columns[i].Width, r.Top + headerBuffer);

                        if (headerStyle == ColumnHeaderStyle.Clickable && columns[i].Pressed)
                        {
                            if (this.headerImageObject.DownBitmap != null)
                            {
                                DrawEngine.DrawRect2(g, this.headerImageObject, buttonRect, ControlState.Down);
                            }
                            else
                            {
                                ControlPaint.DrawButton(g, lp_scr + last, r.Top + 1, columns[i].Width, r.Top + headerBuffer, ButtonState.Flat);
                            }
                        }
                        else if (headerStyle == ColumnHeaderStyle.Clickable && columns[i].Hovered)
                        {
                            if (this.headerImageObject.HighlightBitmap != null)
                            {
                                DrawEngine.DrawRect2(g, this.headerImageObject, buttonRect, ControlState.Highlight);
                            }
                            else
                            {
                                ControlPaint.DrawButton(g, lp_scr + last, r.Top + 1, columns[i].Width, r.Top + headerBuffer, ButtonState.Flat);
                        ...
                        {
                            if (this.headerImageObject.NormalBitmap != null)
                            {
                                DrawEngine.DrawRect2(g, this.headerImageObject, buttonRect, ControlState.Normal);
                            }
                            else
                            {
                                System.Windows.Forms.ControlPaint.DrawButton(g, lp_scr + last, r.Top + 1, columns[i].Width, r.Top + headerBuffer, ButtonState.Normal);
                            }
                        }

                        g.DrawLine(new Pen(this.headerSplitColor), new Point(buttonRect.Right - 1, buttonRect.Top), new Point(buttonRect.Right - 1, buttonRect.Bottom));
					
						if (columns[i].Image != null)
						{
                            int y = (rowHeight - this.columns[i].Image.Height) / 2 + 2;

                            Rectangle imgRect = new Rectangle(lp_scr + last + 4, y, rowHeight - 4, rowHeight - 4);
                            g.DrawImage(columns[i].Image, imgRect, 0, 0, columns[i].Image.Width, columns[i].Image.Height, GraphicsUnit.Pixel);
                            g.DrawString(TruncatedString(columns[i].Text, columns[i].Width, 25, g), this.Font, new SolidBrush(this.ForeColor), (float)(lp_scr + last + 22), (float)(r.Top + 5));
						}
						else
						{
							string sp = "";
                            ...
                            if (columns[i].TextAlign == HorizontalAlignment.Left)
                            {
                                ...
                                g.DrawString(TruncatedString(columns[i].Text, columns[i].Width, 0, g), this.Font, new SolidBrush(this.ForeColor), buttonRect, strFmt);
                            }
                            else if (columns[i].TextAlign == HorizontalAlignment.Right)
                            {
                                strFmt.Alignment = StringAlignment.Far;
                                sp = TruncatedString(columns[i].Text, columns[i].Width, 0, g);
                                g.DrawString(sp, this.Font, new SolidBrush(this.ForeColor), buttonRect, strFmt);
                            }
                            else
                            {
				...
				{
                    Rectangle leftRect = new Rectangle(lp_scr + last, r.Top + 1, r.Width - (r.Left + last) - 3 + hscrollBar.Value, r.Top + headerBuffer);
                    g.Clip = new Region(leftRect);
                    if (this.headerImageObject.NormalBitmap != null)
                    {
                        DrawEngine.DrawRect2(g, this.headerImageObject, leftRect, ControlState.Normal);
                    }
                    else
                    {
		...
		protected virtual void DrawRows(Graphics g, Rectangle r)
		{
			if (columns.Count > 0)
			{
				...
				for (i=0; i<items.Count; i++)
				{
					...
					{
						g.Clip = new Region(new Rectangle(r.Left+1, r.Top+headerBuffer+1, r.Width-vsize-4, r.Height-hsize-5));
                        ...
                        Rectangle rowRect = new Rectangle(lp, tp_scr+(rowHeight*i), rowSelWidth, rowHeight);
						if (items[i].Selected && isFocused)
						{
                            if (this.rowImageObject.CheckedBitmap != null)
                                DrawEngine.DrawRect2(g, this.rowImageObject, rowRect, ControlState.Checked);
                            else
							    g.FillRectangle(new SolidBrush(rowSelectColor), rowRect);
						}
						...
						{
                            if (this.rowImageObject.HighlightBitmap != null)
                                DrawEngine.DrawRect2(g, this.rowImageObject, rowRect, ControlState.Down);
                            else
							    g.FillRectangle(SystemBrushes.Control, lp, tp_scr+(rowHeight*i), rowSelWidth, rowHeight);
						}

                        // hot-tracked row
                        if (items[i].Hovered && this.hoverSelection/*doRowTracking && (lastRowHovered >= 0 && lastRowHovered < items.Count)*/)
                        {
                            if (this.rowImageObject.HighlightBitmap != null)
                                DrawEngine.DrawRect2(g, this.rowImageObject, rowRect, ControlState.Highlight);
                            else
                                g.FillRectangle(new SolidBrush(rowTrackColor), rowRect);
                        }
						...
						{	
							...
							{
                                Image itemImage = smallImageList.Images[items[i].ImageIndex];
                                int y = 1;
                                int imgWidth = rowHeight - y * 2;
                                int imgHeight = imgWidth;

                                ImageAttributes imgAttr = new ImageAttributes();
                                imgAttr.SetColorKey(smallImageList.TransparentColor, smallImageList.TransparentColor);
                                Rectangle imgRect = new Rectangle(lp_scr + 4, tp_scr + (rowHeight * i) + y, imgWidth, imgHeight);
                                g.DrawImage(itemImage, imgRect, 0, 0, itemImage.Width, itemImage.Height, GraphicsUnit.Pixel, imgAttr);

							//	smallImageList.Draw(g, lp_scr+4, tp_scr+(rowHeight*i)+1, 16, 16, items[i].ImageIndex);	
                                Rectangle textRect = new Rectangle(lp_scr + imgWidth + 6, tp_scr + (rowHeight * i) + y, columns[j].Width, imgHeight);
                                StringFormat strFmt = new StringFormat();
                                strFmt.Alignment = StringAlignment.Near;
                                strFmt.LineAlignment = StringAlignment.Center;
                                string truncatedStr = TruncatedString(items[i].Text, columns[j].Width, 18, g);
                                g.DrawString(truncatedStr, this.Font, (items[i].Selected && isFocused ? SystemBrushes.HighlightText : new SolidBrush(ForeColor)),
                                    textRect, strFmt);
							}
							else
							{
