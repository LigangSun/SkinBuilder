...
namespace ZLIS.SkinBuilder
{
	...
	public class TreeListNode: IParentChildList
	{
        ...
        public EventHandler<EventArgs> OnSelectedChanged;
		...
		public TreeListNode()
		{
			...
			nodes = new TreeListNodeCollection(this.treelistview);
			nodes.Owner = this;
		...
		public void Collapse()
		{
            ...
            if (this.TreeListView != null)
                this.TreeListView.AdjustScrollbars();
		}
	...
	public class TreeListNodeCollection: CollectionBase
	{
        ...
        public TreeListNodeCollection(TreeListView view)
		{
            this.treeListView = view;
		}

		public TreeListNodeCollection(TreeListNode owner, TreeListView view)
		{
			this.owner = owner;
            this.treeListView = view;
		}
		...
		public TreeListNode this[int index]
		{
			...
			set 
			{
				...
				tln.Parent = owner;
                tln.TreeListView = this.treeListView;
				OnNodesChanged();
			}
		}
		...
		public int Add(TreeListNode item)
		{
			...
			item.Parent = owner;
            item.TreeListView = this.treeListView;
			OnNodesChanged();
		...
		public void AddRange(TreeListNode[] items)
		{
			lock(List.SyncRoot)
			{
				for (int i=0; i<items.Length; i++)
				{
                    ...
                    items[i].TreeListView = this.treeListView;
				}
				OnNodesChanged();
			}
		}
	...
	public class TreeListView : ContainerListView
	{
		...
		public TreeListView(): base()
		{
			...
			selectedNodes = new TreeListNodeCollection(this);
        ...
        private TreeListNode hoverNode = null;
		...
		public override void AdjustScrollbars()
		{
			if (nodes.Count > 0 || columns.Count > 0 && !colScaleMode)
			{
				...
				vscrollBar.Top = this.ClientRectangle.Top+headerBuffer+1;
		...
		private void RenderNodeRows(TreeListNode node, Graphics g, Rectangle r, int level, int index, ref int totalRend, ref int childCount, int count)
		{
			if (node.IsVisible)
			{
					...
					&& (r.Top+itemheight*totalRend+eb/4-vscrollBar.Value < r.Top+r.Height)))
				{
                    ...
                    int srWidth = allColsWidth - (lb + ib + eb + 4);
                    int x = r.Left + lb + ib + eb + 4 - hscrollBar.Value;
                    if (this.fullRowSelect)
                    {
                        ...
                        if (this.vscrollBar.Visible)
                            srWidth -= this.vscrollBar.Width;
                    }
                    Rectangle sr = new Rectangle(x, r.Top + hb + itemheight * totalRend - vscrollBar.Value, srWidth, itemheight);
					...
					if (node.Selected && isFocused)
					{
                        if (this.rowImageObject.CheckedBitmap != null)
                            DrawEngine.DrawRect2(g, this.rowImageObject, sr, ControlState.Checked);
                        else
                    ...
                    if (node.Hovered)
                    {
                        if (this.rowImageObject.HighlightBitmap != null)
                            DrawEngine.DrawRect2(g, this.rowImageObject, sr, ControlState.Highlight);
                        else
                            g.FillRectangle(new SolidBrush(rowTrackColor), sr);
                    }
                    ...
                    else
                    {
                        ...
                        {
                            Image itemImage = smallImageList.Images[node.ImageIndex];
                            int y = 1;
                            ...
                            Rectangle imgRect = new Rectangle(r.Left + lb + eb + 2 - hscrollBar.Value,
                                r.Top + hb + itemheight * totalRend + eb / 4 - 2 - vscrollBar.Value,
                                imgWidth, imgHeight);
					...
					{
                        string truncateStr = TruncatedString(node.Text, columns[0].Width, lb + eb + ib + 6, g);

                        StringFormat strFmt = new StringFormat();
                        strFmt.LineAlignment = StringAlignment.Center;
                        strFmt.Alignment = StringAlignment.Near;
                        Brush textColorBrush = new SolidBrush(this.ForeColor);
                        if (node.Selected && isFocused)
                            textColorBrush = SystemBrushes.HighlightText;

                        int left = r.Left + lb + ib + eb + 4 - hscrollBar.Value;
                        int top = r.Top + hb + itemheight * totalRend/* + eb / 4*/ - vscrollBar.Value;
                        Rectangle textRect = new Rectangle(left, top,
                            columns[0].Width - left, itemheight);
                        g.DrawString(truncateStr, this.Font, textColorBrush, textRect, strFmt);
                        //if (node.Selected && isFocused)
                        //    g.DrawString(truncateStr, Font, SystemBrushes.HighlightText, (float)(r.Left+lb+ib+eb+4-hscrollBar.Value), (float)(r.Top+hb+itemheight*totalRend+eb/4-vscrollBar.Value));
                        //else
						...
						for (j=0; j<node.SubItems.Count && j<columns.Count; j++)
						{
							...
							else
							{
								...
								else if (columns[j+1].TextAlign == HorizontalAlignment.Right)
								{
									...
									if (node.Selected && isFocused)
										g.DrawString(sp, this.Font, SystemBrushes.HighlightText, (float)(last+columns[j+1].Width-StringTools.MeasureDisplayStringWidth(g, sp, this.Font)-4-hscrollBar.Value), (float)(r.Top+(itemheight*totalRend)+headerBuffer+4-vscrollBar.Value));
									else
										g.DrawString(sp, this.Font, (node.UseItemStyleForSubItems ? new SolidBrush(node.ForeColor) : SystemBrushes.WindowText), (float)(last+columns[j+1].Width-StringTools.MeasureDisplayStringWidth(g, sp, this.Font)-4-hscrollBar.Value), (float)(r.Top+(itemheight*totalRend)+headerBuffer+4-vscrollBar.Value));
								}
								else
								{
									sp = TruncatedString(node.SubItems[j].Text, columns[j+1].Width, 9, g);
									if (node.Selected && isFocused)
										g.DrawString(sp, this.Font, SystemBrushes.HighlightText, (float)(last+(columns[j+1].Width/2)-(StringTools.MeasureDisplayStringWidth(g, sp, this.Font)/2)-hscrollBar.Value), (float)(r.Top+(itemheight*totalRend)+headerBuffer+4-vscrollBar.Value));
									else
										g.DrawString(sp, this.Font, (node.UseItemStyleForSubItems ? new SolidBrush(node.ForeColor) : SystemBrushes.WindowText), (float)(last+(columns[j+1].Width/2)-(StringTools.MeasureDisplayStringWidth(g, sp, this.Font)/2)-hscrollBar.Value), (float)(r.Top+(itemheight*totalRend)+headerBuffer+4-vscrollBar.Value));
								}
							}
						}
					}
		...
		{
			TreeListNodeCollection list = new TreeListNodeCollection(this);
